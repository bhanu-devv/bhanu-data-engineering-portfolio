...
| **9.5** | Cinematic experience upgrade (**complete**) | `Intro.tsx` (`src/components/intro/`) — a one-time cinematic opening layered on top of Phase 9's finished site, not a rebuild of it: name/role/"Connect" idle state → a small network (`IntroNetwork.tsx`, the same 5 curated `heroNetwork` technologies and angles `HeroWeb` already uses) forms via staggered opacity/transform → the whole overlay crossfades away, revealing the already-rendered Header/Hero underneath (§15 D73's "matching geometry + crossfade, not literal DOM morphing" reasoning). Session-once via `sessionStorage`, bypassed entirely (never mounted) for `prefers-reduced-motion: reduce`, a meaningful deep-link hash, or a same-session return visit; a real "Skip intro" control; total connect-to-Hero duration ≈1.1s | Lint, typecheck, `content:check` (both), `resume:check`, and build all pass; real Playwright-MCP QA verified keyboard activation (Tab lands on Connect, Enter activates), all three bypass paths (reduced-motion, `#projects`/`#contact`/`#resume`, valid and invalid `#project=` deep links — confirmed only with genuine fresh navigations, since same-document hash-only navigations do not reset the intro's browser-state decision, a real methodological pitfall found and corrected during this phase's own testing), Skip (instant dismissal, focus moves to the skip link), and session persistence (sessionStorage-clear correctly replays the intro, matching a new browser session); all 7 breakpoints clean (no overflow, mobile shows "Tap to connect" vs. desktop's "Click to connect" via a pure CSS media toggle, no client JS needed to pick the string); zero console/hydration errors; JS transfer +1,987 bytes over the Phase 9 baseline (146,951 vs. 144,964 bytes), still well under the ~170 KB budget, entirely `Intro.tsx`'s own cost — no dependency was added. Two real bugs were found and fixed during implementation, not just at QA: (1) a `pathLength`-normalized `stroke-dasharray`/`stroke-dashoffset` did not reliably hide a `<line>` inside a `preserveAspectRatio="none"` container across engines, visible as faint stray strands during the idle state — fixed by switching the strand draw-in to a plain opacity transition (§15 D74); (2) marking `sessionStorage` "seen" at the moment Connect is clicked raced with `useSyncExternalStore`'s tearing-check re-invoking `getSnapshot` on the very next render the animation's own state update caused, silently skipping straight to "done" and killing the animation before it could play — fixed by caching the decision after its first computation, since "should this page load show the intro" must stay fixed for that load regardless of writes the load itself makes (§15 D75) |
| **9.6** | Content depth, credibility & scannability (**complete**) | **Scroll-for-overview / click-for-depth**, extended from Projects (Phase 6) to Experience and Leadership: every role/leadership card's collapsed state now shows only `summary` + one `impact`/`scope` line + `tech`, with the richer detail (the same already-approved `bullets`, reorganized into `expanded`'s readable named groups — §15 D77) behind a native `<details>`/`<summary>` "View full experience" / "View leadership details" (§15 D76, zero new dependency, zero new client JS). Hero proof strip revised to a new combined `azure-platform-scope` metric leading the same four-item strip (§15 D83). `ProjectStatusBadge`'s `"in-progress"` label changed to "Current Build" (§15 D84) — the enum itself untouched, so flipping to `"complete"` later needs one content edit. GPSA leadership gained real `summary`/`scope`/`expanded` content from Bhanu-approved LinkedIn material, dates deliberately unchanged (§15 D78). The Recommendations section now renders for the first time on the real site: one real, approved manager recommendation (Joseph Minerd) with a verbatim `quote`, a literal-substring `excerpt`, and a clearly-labeled `summary` (§15 D79), `blockquote`/`cite` semantics preserved, revealed via the same native-disclosure pattern. ENERGY STAR gained a confirmed `links.github`; Steam Bill Automation's stayed intentionally unset — no guessed URL (§15 D80). Both Oracle credentials gained real `issued`/`expires`/`credentialId`/`verifyUrl`, rendered as quiet metadata plus a "Verify credential" link, credential ID itself never displayed (§15 D81) | Lint, typecheck, `content:check` (both), `resume:check`, and build all pass; deliberately **no new content-check rules** (§15 D82) — every genuinely checkable new-content invariant was already enforced by existing `.url()` schema constraints and the pre-existing unapproved-recommendation rule; real Playwright-MCP QA verified keyboard + mouse toggling of every disclosure (Experience ×3, Leadership, Recommendation), a visible focus ring on every `<summary>`, the full recommendation's exact 591-character match against the approved source, the ENERGY STAR GitHub URL and both Oracle verification URLs character-for-character, zero fifth proof-strip item and zero "6,540" anywhere on the site, the flagship project's "Current Build" badge and zero "Implemented" milestone claims (both on the card and in the case-study modal), and full Phase 9/9.5 regression (intro, Skip, session behavior, reduced-motion bypass, header/mobile-menu, project modal open/close, Resume/Contact/404/OG/JSON-LD) all intact; all 7 breakpoints clean with every disclosure expanded simultaneously; JS transfer +50 bytes over the Phase 9.5 baseline (147,001 vs. 146,951 bytes) — effectively flat, since every new interaction is native `<details>`/`<summary>` |
| **9.7** | Ambient motion and interaction polish (**complete**) | `CursorGlow.tsx` (`src/components/effects/`) — a soft pointer-following radial light, one component mounted once, gated to `pointer:fine` + no-reduced-motion (renders `null` entirely otherwise — §15 D89), writing `--pointer-x`/`--pointer-y` inside `requestAnimationFrame` exactly like `useTilt` already does, appearing only after the first real pointer move. A static (no continuous loop) `NetworkMesh` backdrop added to Projects, Skills, and Contact — Hero already had one via `HeroWeb` — reusing the existing primitive rather than building a new one (§15 D86); two real bugs found and fixed during implementation: a negative `z-index` painted the backdrop behind the page's own root background (§15 D87), and the first opacity value (0.07) was technically correct but visually imperceptible (§15 D88, raised to 0.14). Project cards and Skills hub cards get a quiet `:hover`/`:focus-within` border-color response (crimson) plus, for project cards, a small activation dot — pure CSS, keyboard focus gets the identical treatment as mouse hover (§15 D90/D91). The project case-study architecture diagram's connectors now activate once on mount (which only ever happens when the modal opens) via a single `useState` flip in a `requestAnimationFrame` callback (§15 D92) | Lint, typecheck, `content:check` (both), `resume:check`, and build all pass; real Playwright-MCP QA verified the cursor glow appears only after pointer movement and correctly never mounts under `prefers-reduced-motion: reduce` or (confirmed via Claude Browser's touch-emulated mobile preset, `pointer: fine` genuinely `false`) a touch device; project-card and Skills-hub hover AND keyboard-focus both produce the identical crimson border-color change; the architecture diagram's connectors reach full opacity after modal open, and correctly skip the transition entirely (instant, still fully visible) under reduced motion; all 7 breakpoints clean with the ambient backdrop present; zero console errors, zero extra network requests from pointer movement; full Phase 9/9.5/9.6 regression (intro, Skip, session behavior, deep-link bypass, nav, mobile menu, project modals, ENERGY STAR GitHub, Oracle verification, Resume, Contact, 404, OG, JSON-LD) all intact; JS transfer +246 bytes over the Phase 9.6 baseline (147,247 vs. 147,001 bytes), no dependency added |
...
| **11** | Publish (**complete, approved by Bhanu**) | Public repo `bhanu-devv/bhanu-data-engineering-portfolio` (only `main` pushed — no tags, no other branches; the rejected Phase 10 experiment branch and its stash stay local); Vercel production at `https://bhanu-data-engineering-portfolio.vercel.app`, deployed via the Vercel CLI (the GitHub-app auto-deploy connection was not set up); `site.seo.url` set to that URL, read everywhere through a new `getSiteUrl()` selector (`metadataBase`, canonical, `og:url`, JSON-LD `url`/`image`, and new `robots.ts`/`sitemap.ts`), each omitted cleanly when the URL is `needsInput()`; README gained a Live Portfolio link. No custom domain. **Update (post-publish):** the production URL is now `https://bhanu-n.vercel.app` (`site.seo.url`, README, and the repo homepage updated in `chore: update production domain`); the original alias above 307-redirects to it, and Vercel's GitHub integration now auto-deploys `main` | Full privacy/history audit of every path and line in `main`'s history passed before the first push | **Pre-checks:** `git log --format='%an <%ae> | %cn <%ce>'` shows only the noreply address across all commits (identity already corrected, D28); Bhanu has enabled GitHub's "Block command line pushes that expose my email" setting. (The public-PDF phone question is resolved by decision 19.) Explicit go-ahead for each of: repo creation, push, deploy, domain |
...
| D73 | The intro's network "becomes" the Hero network through matching geometry (the same 5 curated `heroNetwork` technologies, the same 5 angles `HeroWeb` places them at) plus a simple crossfade, not a literal shared-element DOM morph between two independently-rendered trees | Decided in Phase 9.5 | The intro overlay and `HeroWeb` are unrelated component trees, rendered in different containers, at different sizes, across different breakpoints — synchronizing their exact pixel positions for a true morph would need fragile viewport-relative coordinate math recomputed on every resize, for a ~1.2 s effect the crossfade itself already sells perceptually. Centered placement + identical visual vocabulary (node style, strand style, the single live-crimson node, the same five names) achieves the "the network becomes the Hero" narrative Step 4 asked for, robustly, at every breakpoint, without that fragility — the same "prefer the simpler mechanism that achieves the same read" reasoning as D50/D60/D68 |
| D74 | The intro's network strands fade in via `opacity` (matching the nodes), not a `stroke-dashoffset` "draw" the way `NetworkMesh`'s unfurl works | Decided in Phase 9.5, found via direct visual inspection — faint strand fragments were visible during the supposedly-hidden idle state | The intro's `<svg>` uses `preserveAspectRatio="none"` (needed so the network isn't letterboxed at the overlay's own aspect ratio, unlike `HeroWeb`'s square container). A `pathLength`-normalized `stroke-dasharray`/`stroke-dashoffset` did not reliably compute as fully hidden against that non-uniform scaling in testing; `getTotalLength()` → a `--path-length` custom property (`NetworkMesh`'s own technique) was tried next but added a client-only mount effect for a cosmetic difference from opacity that testing couldn't actually distinguish once the per-strand stagger was in place — opacity alone, with no scaling-related failure mode, reads as "network forming" just as clearly |
| D75 | `Intro.tsx`'s show/skip decision (`useSyncExternalStore`'s `getSnapshot`) is cached after its first real computation, in a module-level variable, rather than recomputed live on every call | Decided in Phase 9.5, found by Playwright-driven interaction testing — clicking Connect appeared to do nothing, the overlay vanishing almost instantly instead of playing the ~1.1 s animation | `getSnapshot` reads `sessionStorage`, and `handleConnect` writes to that same key (`markSeen()`) the instant Connect is activated — `useSyncExternalStore` re-invokes `getSnapshot` on every render regardless of `subscribe` (it checks for tearing even with a no-op subscribe), so the animation's own first `setLocalPhase("connecting")` triggered a re-render that re-read `sessionStorage`, now seeing the write it had just caused, and flipped the decision from "show" to "skip" mid-flight. Caching the decision the first time it's computed fixes this correctly, not just symptomatically: "should this page load show the intro" is a fact about how the load *started*, and marking the session seen is deliberately about the *next* load, not a live condition this load should keep re-checking |
| D76 | Experience/Leadership/Recommendations' inline "View more" disclosures use native `<details>`/`<summary>`, not a client-side accordion — zero new dependency, zero new client JS | Decided in Phase 9.6 | The Phase 9.6 brief explicitly preferred native `<details>`/`<summary>` "if it integrates cleanly," and it does: native toggling is instant (no height/opacity animation to gate under reduced-motion — there is none), keyboard/focus/expanded-state semantics come for free, and every collapsed-card field (summary, one impact/scope line, tech) was already server-rendered content, not new UI |
| D77 | `content/experience.ts`'s new `expanded` groups (Part 4) reuse the SAME sentences already in each role's existing, approved `bullets` — reorganized under short headings, not rewritten or supplemented with new claims | Decided in Phase 9.6 | Part 4 asked for "readable grouping," not new facts; grouping the already-resume-sourced bullets keeps every word traceable to the same source `bullets` has always cited, with zero new fact-checking surface. Vipany's and Laxmi's group headings name review/analysis work specifically (never "pipelines" or "platforms") per Parts 6-7's explicit instruction not to reframe either role as a Data Engineering position |
| D78 | GPSA leadership gained real `summary`/`scope`/`expanded` content from Bhanu-approved LinkedIn material (Graduate Gala Night, WhirlyBall, 100+ graduate students, Programming Board coordination) — but its `start`/`end` dates were left unchanged at the already-validated resume values, not updated to LinkedIn's different start date | Decided in Phase 9.6, per Bhanu's explicit instruction | Bhanu named LinkedIn as an approved ADDITIONAL source for facts LinkedIn alone supports (the event/scope detail resume space didn't allow for), but explicitly not an authority that overrides already-validated resume/master values on conflict — the master date stays authoritative until Bhanu separately approves changing it |
| D79 | The real approved manager recommendation (Joseph Minerd) is published with three distinct text fields — `quote` (the complete, verbatim recommendation), `excerpt` (an exact substring of `quote`, never a paraphrase), and `summary` (clearly-labeled prose, rendered outside any blockquote/quotation marks) | Decided in Phase 9.6, explicit Bhanu approval (publication + the author's own LinkedIn recommendation as permission) | CLAUDE.md §2 rule 5 forbids "paraphrase, 'improve', or excerpt-and-alter" a recommendation; keeping `excerpt` as a literal substring (not a rewrite) and `summary` visually and semantically distinct from the quote (never in quotation marks) is what makes that rule mechanically true rather than a matter of writing carefully at content-authoring time |
| D80 | The ENERGY STAR project's flagship-card/case-study "View on GitHub" action now links to `https://github.com/bhanu-devv/csu-energystar-automation`; the Steam Bill Automation project's `links.github` stays unset | Decided in Phase 9.6, per Bhanu's explicit confirmation for ENERGY STAR and explicit instruction NOT to guess a URL for Steam Bill | Bhanu's LinkedIn material confirmed a direct ENERGY STAR repository URL but only a LinkedIn short-link (not a verified direct GitHub URL) for Steam Bill — CLAUDE.md's "never invent a link" rule applies exactly as strictly to guessing a plausible-looking repo slug as it does to any other fact; `links.github` stays absent (self-hiding, no dead or wrong link) until Bhanu supplies the real URL |
| D81 | Both Oracle credentials gained real `issued`/`expires`/`credentialId`/`verifyUrl` values from Bhanu-approved official Oracle verification pages; the credential ID itself is stored but deliberately never rendered in the UI | Decided in Phase 9.6 | Part 16's own instruction: "verification links matter more than showing a long ID." `Certifications.tsx` renders `issued`/`expires` as quiet metadata and "Verify credential" when `verifyUrl` is set, exactly the existing self-hiding-per-field pattern Phase 7 already established — no redesign toward a "badge wall" |
| D82 | No new `content-check.ts` validation rules were added in Phase 9.6, despite Part 32 inviting them | Decided in Phase 9.6 | Every rule Part 32 suggested (recommendation approved before rendering, non-empty quote, valid recommendation/project-GitHub/verifyUrl URLs) was already enforced — by the pre-existing "unapproved recommendation" ERROR rule (Phase 2) and by `.url()` constraints already on every relevant Zod schema field. Matches D62's Phase 7 precedent: adding a rule with nothing new to guard against is checkbox-driven, not genuinely useful |
| D83 | The Hero proof strip's first item combines what were two separate metrics (`campus-buildings`, `monthly-bill-items`) into one new record, `azure-platform-scope` ("Azure Utility Data Platform" / "20+ campus buildings · 170+ monthly bill-related items") — the two original metric records stay in `content/metrics.ts`, simply no longer selected for the strip | Decided in Phase 9.6 Part 1 | Keeps the strip at exactly four items while representing platform scope as one coherent fact rather than two separate building/item-count numbers; deleting the two original records outright risked losing content a future phase might still want (e.g. a project-level `metricIds` reference), so they were left in place, unused, rather than removed |
| D84 | `ProjectStatusBadge`'s `"in-progress"` label changed from "In Progress" to "Current Build" — the only change; the underlying `status` enum (`"in-progress" \| "complete"`) is untouched | Decided in Phase 9.6 Part 2 | Bhanu's explicit instruction: no "Ongoing Project" repeated site-wide, and a status wording that reads as more deliberate/professional than "in progress" while staying honest that the work isn't finished. Because only the *label* changed, flipping the flagship project to `"complete"` later (whenever it truly is) needs one content edit (`content/projects.ts`'s `status:` field) and nothing else — the badge already knows how to render "Complete" |
| D85 | LinkedIn/master-content conflict policy (Phase 9.6 pre-check): LinkedIn is an approved ADDITIONAL source for facts the resume's space constraints left out (event names, GitHub URLs, official credential verification, richer leadership detail) — but the existing validated resume/portfolio value is always authoritative on direct conflict, never silently overwritten. Found and left unresolved, exactly as instructed: CSU Data Analyst start date (LinkedIn: Oct 2025; master: Aug 2025), GPSA start date (LinkedIn: May 2025; master: Aug 2025), master's program name (LinkedIn: "Information Technology"; master resume: "Information Systems" — kept), education dates generally, and the utility-record count (LinkedIn: 6,540 in multiple places; master: 6,854+ — kept, not averaged, not shown as two numbers). None of these were changed | Decided in Phase 9.6 | This is the same "never invent, never silently reconcile a source conflict" discipline CLAUDE.md §2 already applies to every other fact, extended explicitly to a second source now in play; a future phase should keep treating LinkedIn the same way — supplementary, never overriding, every conflict reported rather than resolved unilaterally |
| D86 | The ambient network backdrop (Projects, Skills, Contact) reuses `NetworkMesh` as-is — no new "AmbientBackground" component was built, and no continuous CSS animation was added to it | Decided in Phase 9.7 | `NetworkMesh` already draws in once then stays fully static (its own doc comment: "nothing here loops"), is already reduced-motion-safe, and is already the site's one canonical "atmospheric web" primitive (used in `HeroWeb` since Phase 4) — literally the "one reusable ambient background primitive rather than separate unrelated effects" the brief asked for. Adding a second, continuously-drifting background layer would also have risked a real CLAUDE.md §5 budget-rule-2 violation (competing with `Node`'s "live" pulse for the one permitted continuous ambient loop); a static backdrop sidesteps that entirely |
| D87 | The three `NetworkMesh` backdrop instances (Projects/Skills/Contact) use `absolute inset-0` with no explicit `z-index` (relying on DOM order — mounted first, before the section's real content) rather than a negative `z-index` (`-z-10`) | Decided in Phase 9.7, found via direct computed-style inspection after the backdrop rendered fully invisible in testing | A negative `z-index` on a `position: absolute` descendant of a `position: relative` ancestor that does **not** itself establish a stacking context (`position: relative` alone, no explicit `z-index`, is not enough) can paint behind the page's own root background, not just behind its sibling content — exactly what happened here. Removing the negative z-index and relying on paint order (first child, `z-index: auto`) keeps the mesh reliably between the void background and the section's real content at every breakpoint, with no stacking-context fragility |
| D88 | The ambient backdrop's opacity is `0.14`, not the `0.07` first tried | Decided in Phase 9.7, found via direct visual inspection | At `0.07`, `--color-grid` (steel-500) composited against the void background produced a color difference small enough to be imperceptible in practice, even though every computed style (opacity, stroke, drawn state) was technically correct — "extremely restrained" was being satisfied to the point of not registering as a visual effect at all. `0.14` is the minimum bump that reads as a genuine, if still very quiet, presence |
| D89 | The cursor-following light (`CursorGlow.tsx`) is one component, mounted once at the page root, driving a single `position: fixed` full-viewport gradient layer via `--pointer-x`/`--pointer-y` — not a per-section effect, and not React state | Decided in Phase 9.7 | Matches `useTilt`'s already-established pattern exactly (CLAUDE.md §5 budget rule 3: "write CSS variables... inside requestAnimationFrame... never trigger React re-renders per mouse move") — one `window`-level `pointermove` listener is cheaper than N per-section listeners, and a single shared layer is simpler to reason about for the reduced-motion/touch gating this effect needs (Parts 5, 18, 19): the component renders `null` entirely in those cases, not just a CSS-hidden node |
| D90 | Project-card and Skills-hub hover/focus polish (Parts 8, 10) is pure CSS (`:hover`/`:focus-within` border-color and opacity transitions) — no JavaScript, no new client components | Decided in Phase 9.7 | Both are simple state-driven style changes with no continuous motion and no data to track; CSS pseudo-classes already do this natively and instantly respect `prefers-reduced-motion` for the transition itself via the existing `@media (prefers-reduced-motion: reduce) { transition: none }` pattern this codebase already uses everywhere else (depth.css's `.tilt`) |
| D91 | The Skills section's "connector opacity increases on hover" idea (Part 10) was narrowed to "a hovered/focused group's own hub card border brightens" — the shared connector row above the grid does not react to individual card hover | Decided in Phase 9.7 | The connector is one shared row spanning all five cards, rendered as an earlier DOM sibling of the grid — a plain CSS sibling combinator cannot reach "backward" from five separate, individually-hovered later cards to one shared earlier element. Per-card self-contained hover (matching the same treatment Project cards already got) achieves the same "this group is now active" read without inventing a JS-driven cross-element wiring mechanism for a purely decorative response |
| D92 | The architecture diagram's connector activation (Part 9) is a `useState` flip inside a `useEffect`+`requestAnimationFrame` on mount, not a CSS-only `.js`-class-gated animation like `NetworkMesh`'s own unfurl | Decided in Phase 9.7 | `ArchitectureDiagram` only ever mounts when the case-study modal opens (it's lazy-loaded inside `ProjectModalContent`), so "on mount" already means "on modal open" — there is no separate open/close event to listen for. The component was already going to need `"use client"` for this reason regardless of technique; a `useState` set inside an rAF callback (not synchronously in the effect body) matches the `react-hooks/set-state-in-effect` lint rule's own carve-out ("calling setState in a callback function when external state changes") the same way D57's `ProjectsGrid` hash-reading fix does |
